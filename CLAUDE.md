...
- Legacy provider files (`groq.ts`, `vertex.ts`, `anthropic.ts`) still carry `// @ts-nocheck` markers and need a clean port — listed in [apps/api/tsconfig.json](apps/api/tsconfig.json). `ollama.ts` (Ollama + LM Studio) has been ported and is type-checked. OpenAI/Azure/Vertex constructors now honor settings-store provider config.
